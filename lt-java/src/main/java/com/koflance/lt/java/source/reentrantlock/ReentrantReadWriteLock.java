package com.koflance.lt.java.source.reentrantlock;
...
public class ReentrantReadWriteLock
     ...
     */
    abstract static class Sync extends com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer {
        private static final long serialVersionUID = 6317671515068378041L;

        // 第16位作为共享锁和排他锁的分界线，高16位表示共享锁个数，低16位表示排他锁个数
        static final int SHARED_SHIFT   = 16;
        // 每次增加共享锁的单位compareAndSetState(c, c + SHARED_UNIT)
        static final int SHARED_UNIT    = (1 << SHARED_SHIFT);
        // 允许的最大锁个数
        static final int MAX_COUNT      = (1 << SHARED_SHIFT) - 1;
        // 排他锁掩码，从0~15位，表示排他锁个数
        ...
        protected final int tryAcquireShared(int unused) {
             ...
             * 工作流程:
             * 1. w锁被其他线程持有，返回失败
             * 2. 否则Otherwise, this thread is eligible for
            ...
            if (!readerShouldBlock() &&
                ...
                // 当前LCH队列中没有比当前线程更前的节点，且共享锁个数小于MAX_COUNT，且设置共享锁的state成功
                if (r == 0) {
                    // 第一个共享锁
                    firstReader = current;
    ...
    public boolean hasWaiters(Condition condition) {
        ...
        if (!(condition instanceof com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject))
            throw new IllegalArgumentException("not owner");
        return sync.hasWaiters((com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject)condition);
    }
    ...
    public int getWaitQueueLength(Condition condition) {
        ...
        if (!(condition instanceof com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject))
            throw new IllegalArgumentException("not owner");
        return sync.getWaitQueueLength((com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject)condition);
    }
    ...
    protected Collection<Thread> getWaitingThreads(Condition condition) {
        ...
        if (!(condition instanceof com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject))
            throw new IllegalArgumentException("not owner");
        return sync.getWaitingThreads((com.koflance.lt.java.source.reentrantlock.AbstractQueuedSynchronizer.ConditionObject)condition);
    }
